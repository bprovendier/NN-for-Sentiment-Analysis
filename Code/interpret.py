...
def get_important_features(background_data_size, test_sample_size, sequence_length):
    """
    Generate report of model's most important features.

    Arguments:
        background_data_size (int): batch size for train data.
        test_sample_size (int): batch size for test data.
        sequence_length (int): length of sequence for train and test data.

    Returns:
        shap_values (list): list of features by importance.
    """
    # load data
    ...
    # load trained model
    model = TSModelGRU(train_df.shape[1])
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=background_data_size, shuffle=False
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--background-data-size", type=int, default=params["background_data_size"]
    )
    parser.add_argument(
        "--test-sample-size", type=int, default=params["test_sample_size"]
    )
    parser.add_argument(
